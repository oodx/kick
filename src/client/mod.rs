...
use crate::sec::{HeaderValidator, UrlValidator};
use std::sync::Arc;
...
impl ApiClientBuilder {
    ...
    /// Add custom header
    pub fn with_header(mut self, key: String, value: String) -> Result<Self> {
        HeaderValidator::validate_header(&key, &value)?;
    ...
    pub async fn build(self) -> Result<ApiClient> {
        ...
        let user_agent = self
            .user_agent
            .unwrap_or_else(|| config.client.user_agent.clone());
...
impl ApiClient {
    ...
    pub async fn get(&self, url: &str) -> Result<String> {
        let _ = UrlValidator::validate(url)?;
        ...
        self.plugin_manager
            .execute_post_request(url, status_code)
            .await?;
    ...
    fn sanitize_filename(filename: &str) -> Result<String> {
        ...
        if filename.contains("..") {
            return Err(ApiError::other(
                "Invalid filename: parent directory references not allowed",
            ));
        }
        ...
        if filename.trim().is_empty() {
            return Err(ApiError::other(
                "Invalid filename: empty filename not allowed",
            ));
        }
...
mod tests {
    ...
    async fn test_client_with_plugins() {
        ...
        plugin_manager
            .register_plugin(Arc::new(crate::plugin::LoggingPlugin::new()))
            .unwrap();
    ...
    async fn test_api_client_builder() {
        ...
        assert_eq!(
            client.custom_headers.get("X-Test-Header"),
    ...
    async fn test_rejects_malformed_url() {
        ...
        let config = Config::new(temp_dir.path().to_path_buf());
        let client = ApiClient::new(config);
        let result = client.get("ht!tp://bad_url").await;
    ...
    fn test_builder_rejects_invalid_header() {
        let temp_dir = TempDir::new().unwrap();
        let config = Config::new(temp_dir.path().to_path_buf());
        let builder = ApiClientBuilder::new().with_config(config);
        let result = builder.with_header("Bad\rKey".to_string(), "value".to_string());
        assert!(result.is_err());
    }
