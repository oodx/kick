...
async fn test_default_headers_integration() {
    ...
    config.client.default_headers.insert("X-Test-Header".to_string(), "test-value".to_string());
    config.client.default_headers.insert("Authorization".to_string(), "Bearer test-token".to_string());
    ...
    let headers = response_json["headers"].as_object()
        .expect("No headers object in response");
    
    // Verify our default headers were included
    assert!(headers.contains_key("X-Test-Header"), "X-Test-Header not found in request");
    assert!(headers.contains_key("Authorization"), "Authorization header not found in request");
...
async fn test_custom_headers_override_defaults() {
    // Create config with default headers
    let mut config = Config::default();
    config.client.default_headers.insert("X-Custom".to_string(), "default-value".to_string());
...
async fn test_post_request_includes_default_headers() {
    // Create config with default headers
    let mut config = Config::default();
    config.client.default_headers.insert("X-API-Key".to_string(), "secret-key".to_string());
    ...
    let response = client.post_json("https://httpbin.org/post", &test_data).await
    ...
    assert!(headers.contains_key("X-Api-Key"), "X-API-Key not found in POST request");
